...
def to_int(value) -> int:
    """Convert Excel cell value to int"""
    if value is None or value == '-' or value == '':
        return 0
...
def generate_recommendations(results: Dict) -> str:
    ...
    if win_rate >= 60:
        doc.append("✅ **Status:** Excellent performance - System is highly profitable")
    elif win_rate >= 50:
        doc.append("✅ **Status:** Good performance - System is profitable")
    elif win_rate >= 40:
        doc.append("⚠️ **Status:** Marginal performance - Needs optimization")
    else:
        doc.append("❌ **Status:** Poor performance - Major improvements required")
    doc.append("")
    doc.append("---")
    doc.append("")
    ...
    sorted_patterns = sorted(patterns, key=lambda x: to_float(x['win_rate']) if x['win_rate'] else 0, reverse=True)

    for p in sorted_patterns:
        pattern_name = p['pattern']
        trades = to_int(p['total_trades']) if p['total_trades'] else 0
        win_rate = to_float(p['win_rate']) if p['win_rate'] else 0
        avg_pnl = to_float(p['avg_pnl']) if p['avg_pnl'] else 0
        avg_days = to_float(p['avg_days_to_target']) if p['avg_days_to_target'] else 0

        # Determine recommendation
        if win_rate >= 65:
            rec = "✅ TRADE (High confidence)"
        elif win_rate >= 55:
            rec = "✅ TRADE (Medium confidence)"
        elif win_rate >= 45:
            rec = "⚠️ TRADE WITH CAUTION"
        else:
            rec = "❌ AVOID or REDUCE SIZE"
    ...
    for r in regimes:
        regime = r['regime']
        trades = to_int(r['total_trades']) if r['total_trades'] else 0
        win_rate = to_float(r['win_rate']) if r['win_rate'] else 0
        avg_pnl = to_float(r['avg_pnl']) if r['avg_pnl'] else 0
    ...
    for c in confidence:
        conf_range = c['range']
        trades = to_int(c['total_trades']) if c['total_trades'] else 0
        win_rate = to_float(c['win_rate']) if c['win_rate'] else 0
        avg_pnl = to_float(c['avg_pnl']) if c['avg_pnl'] else 0
    ...
    for y in yearly:
        year = y['year']
        trades = to_int(y['total_trades']) if y['total_trades'] else 0
        win_rate = to_float(y['win_rate']) if y['win_rate'] else 0
        avg_pnl = to_float(y['avg_pnl']) if y['avg_pnl'] else 0

        if win_rate >= 55:
            trend = "📈 Strong Year"
        elif win_rate >= 45:
            trend = "➡️ Average Year"
        else:
            trend = "📉 Weak Year"
    ...
    # 1. Pattern filtering
    poor_patterns = [p for p in patterns if to_float(p['win_rate']) < 45]
    if poor_patterns:
        doc.append("### 🔴 Critical: Filter Out Poor-Performing Patterns")
        doc.append("")
        for p in poor_patterns:
            doc.append(f"- **{p['pattern']}**: {to_float(p['win_rate']):.1f}% win rate - Consider DISABLING or requiring higher confidence (≥8.5)")
        doc.append("")
    ...
    if high_conf and low_conf:
        diff = to_float(high_conf['win_rate']) - to_float(low_conf['win_rate'])
        if diff > 10:
            doc.append("### ⚠️ Important: Raise Minimum Confidence Threshold")
            doc.append("")
            doc.append(f"- **High confidence (8.0+):** {to_float(high_conf['win_rate']):.1f}% win rate")
            doc.append(f"- **Low confidence (7.0-7.5):** {to_float(low_conf['win_rate']):.1f}% win rate")
    ...
    best_regime = max(regimes, key=lambda x: to_float(x['win_rate']) if x['win_rate'] else 0)
    worst_regime = min(regimes, key=lambda x: to_float(x['win_rate']) if x['win_rate'] else 0)

    if to_float(best_regime['win_rate']) - to_float(worst_regime['win_rate']) > 15:
        doc.append("### ⚠️ Important: Optimize Market Regime Filtering")
        doc.append("")
        doc.append(f"- **Best regime ({best_regime['regime']}):** {to_float(best_regime['win_rate']):.1f}% win rate")
        doc.append(f"- **Worst regime ({worst_regime['regime']}):** {to_float(worst_regime['win_rate']):.1f}% win rate")
        doc.append("")
    ...
    doc.append("### ✅ Best Practices Based on Results")
    doc.append("")

    best_patterns = [p for p in patterns if to_float(p['win_rate']) >= 65]
    if best_patterns:
        doc.append("**Patterns to FOCUS on (65%+ win rate):**")
        for p in best_patterns:
            doc.append(f"- {p['pattern']} ({to_float(p['win_rate']):.1f}% win rate)")
        doc.append("")

    good_patterns = [p for p in patterns if 55 <= to_float(p['win_rate']) < 65]
    if good_patterns:
    ...
    doc.append("- High confidence (≥8.5/10): 100% position size")
    ...
    if "DISABLE_POOR_PATTERNS" in recommendations:
        ...
        for p in poor_patterns:
            pattern_win_rate = to_float(p['win_rate'])
    ...
    doc.append("2. **Volume confirmation:** Must have 1.5× average volume")
    ...
    doc.append(f"- **Time stop:** Exit after {int(avg_days * 1.5)} days if no movement (1.5× avg winning trade duration)")
    ...
    for p in sorted_patterns[:3]:  # Top 3 patterns
        doc.append(f"- **{p['pattern']}:** {to_float(p['win_rate']):.1f}% win rate, avg {to_float(p['avg_days_to_target']) if p['avg_days_to_target'] else 0:.0f} days → Trade with confidence")
    doc.append("")

    for p in sorted_patterns[-2:]:  # Bottom 2 patterns
        doc.append(f"- **{p['pattern']}:** {to_float(p['win_rate']):.1f}% win rate → Avoid or require ≥8.5 confidence")
    doc.append("")
    ...
    high_conf_trades = sum(to_int(c['total_trades']) if c['total_trades'] else 0 for c in confidence if '8.0' in c['range'] or '9.0' in c['range'])
    high_conf_wins = sum(to_int(c['wins']) if c['wins'] else 0 for c in confidence if '8.0' in c['range'] or '9.0' in c['range'])
    high_conf_win_rate = high_conf_wins / high_conf_trades * 100 if high_conf_trades > 0 else 0

    doc.append(f"### If We Only Traded Confidence ≥8.0:")
    ...
    doc.append("- ✅ Higher win rate and confidence")
    doc.append("- ⚠️ Fewer trading opportunities (more selective)")
    doc.append("")
    ...
    if win_rate >= 55:
        doc.append(f"✅ System is **profitable** with {win_rate:.1f}% win rate")
        doc.append("- Ready for paper trading with recommended improvements")
    elif win_rate >= 45:
        doc.append(f"⚠️ System is **marginally profitable** with {win_rate:.1f}% win rate")
        doc.append("- Requires improvements before live trading")
    else:
        doc.append(f"❌ System needs **significant improvement** (current: {win_rate:.1f}% win rate)")
    ...
    doc.append("1. ✅ Raise minimum confidence from 7.0 to 8.0")
    if poor_patterns:
        doc.append("2. ✅ Disable or restrict poor-performing patterns")
    doc.append(f"3. ✅ Focus trades on {best_regime['regime']} market regime")
    doc.append("")
...
def main():
    ...
    best_pattern = max(results['patterns'], key=lambda x: to_float(x['win_rate']) if x['win_rate'] else 0)
    print(f"   - Best Pattern: {best_pattern['pattern']}")
    print(f"   - Pattern Win Rate: {to_float(best_pattern['win_rate']):.1f}%")
