...
class OneMinBacktester:
    ...
    def _load_stock_list(self) -> List[str]:
        ...
        try:
            with open(config.STOCK_LIST_FILE, 'r') as f:
                data = json.load(f)
                # Use liquid stocks only (same as production)
                return data['stocks'][:100]  # Limit to first 100 for performance
    ...
    def detect_1min_alerts(self, candles: List[Dict], symbol: str) -> List[Dict]:
        """
        Detect 1-minute alerts in historical candle data.

        Applies same 5-layer filtering as production:
        1. Price threshold (0.85% in 1 min)
        ...
        # Need at least 2 candles for 1-min comparison
        if len(candles) < 2:
            return alerts
        ...
        for i in range(1, len(candles)):
            ...
            # Layer 3: Quality filter - Price >= Rs. 50 (no penny stocks)
            if current_price < self.criteria['min_price']:
                self.stats['filtered_penny_stock'] += 1
                continue

            # Calculate average volume (last 5 candles or available)
            lookback = min(5, i)
            avg_volume = sum(candles[i-j]['volume'] for j in range(1, lookback + 1)) / lookback

            # Layer 2: Volume spike requirement (MANDATORY for 1-min)
            volume_multiplier = current_volume / avg_volume if avg_volume > 0 else 0

            if current_volume < self.criteria['min_volume']:
            ...
            # Layer 4: Cooldown check (10 minutes)
            last_alert_time = self.cooldowns.get(symbol)
            if last_alert_time:
                time_since_last = (timestamp - last_alert_time).total_seconds() / 60
                if time_since_last < self.criteria['cooldown_minutes']:
                    self.stats['filtered_cooldown'] += 1
                    continue

            # Layer 5: Cross-alert deduplication (no 5-min alert in last 3 min)
            last_5min_alert = self.recent_5min_alerts.get(symbol)
            if last_5min_alert:
                time_since_5min = (timestamp - last_5min_alert).total_seconds() / 60
                if time_since_5min <= 3:
                    continue  # Skip, too close to 5-min alert

            # Layer 1: Calculate price changes
            drop_1min = ((prev_price - current_price) / prev_price) * 100
            rise_1min = ((current_price - prev_price) / prev_price) * 100

            # Check for DROP alert
            if drop_1min >= self.criteria['drop_threshold']:
                alerts.append({
                    'symbol': symbol,
                    'direction': 'drop',
                    'timestamp': timestamp,
            ...
            # Check for RISE alert (if enabled)
            if config.ENABLE_RISE_ALERTS and rise_1min >= self.criteria['rise_threshold']:
                alerts.append({
                    'symbol': symbol,
                    'direction': 'rise',
                    'timestamp': timestamp,
    ...
    def backtest_stock(self, symbol: str, from_date: date, to_date: date) -> int:
        ...
        for alert in alerts:
            ...
            logger.info(f"  ✓ {alert['direction'].upper():4s} | {alert['timestamp']} | "
